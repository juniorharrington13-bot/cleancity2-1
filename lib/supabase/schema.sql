...
-- A executer une seule fois dans Supabase SQL Editor (projet ixrebfrxhfapndprujvt)
-- Genere pour matcher exactement les colonnes deja utilisees par l'app Flutter.
-- ============================================================================
...
-- L'utilisateur est-il implique dans la demande (generateur, collecteur assigne,
-- centre destinataire) ou admin ? Utilise par les photos, events et le storage.
...
-- (couvre email/password, OAuth Google, OTP telephone).
...
create table public.addresses (
  ...
  -- Colonne geographique maintenue automatiquement depuis lat/lng (PostGIS).
...
-- ---------------------------------------------------------------------------
-- 6. RPC — recherche geographique (PostGIS)
-- ---------------------------------------------------------------------------
...
-- users: profils visibles par tous les connectes (necessaire pour chat/annuaire),
-- modifiables par leur proprietaire ou un admin.
...
-- addresses: proprietaire uniquement en ecriture ; lecture ouverte aux connectes
-- (necessaire pour afficher le point d'un signalement sur la carte).
...
-- waste_requests: n'importe qui de connecte peut VOIR les points sur la carte ;
-- seul l'auteur (generator) ou un agent (collecteur assigne / centre / admin)
-- peut modifier le statut.
...
-- pickups: le collecteur gere sa propre mission ; generateur/centre/admin peuvent lire.
...
-- waste_request_photos: participants de la demande uniquement.
...
-- processing_events: reserve aux centres pour leurs propres pesees, lisible par
-- les autres participants (generateur/collecteur) et l'admin.
...
-- eco_transactions: chaque utilisateur voit son solde ; seul un centre/admin peut crediter.
...
-- payout_requests: l'utilisateur gere ses propres demandes ; validation reservee a l'admin.
...
-- chat_threads / members / messages: reserve aux membres du fil.
...
-- ---------------------------------------------------------------------------
-- 8. STORAGE (buckets + policies) — voir MediaUploadService
-- ---------------------------------------------------------------------------
...
-- request_photos/requests/{requestId}/{uid}/... : reserve aux participants de la demande.
